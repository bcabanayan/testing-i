...
describe('enhancement success', () => {
    test('enhancement level 0 displays nothing before name', () => {
        expect(enhancer.success({durability: 50, enhancement: 0, name: 'itemName', baseName: 'itemName', namePrefix: ''})).toEqual({durability: 50, enhancement: 1, name:'[+1]itemName', baseName: 'itemName', namePrefix: '[+1]'});
    });
    test('maximum enhancement is level 20 (PEN)', () => {
        expect(enhancer.success({baseName: 'itemName', namePrefix: '[PEN]', name: '[PEN]itemName', enhancement: 20})).toEqual({baseName: 'itemName', namePrefix: '[PEN]', name: '[PEN]itemName', enhancement: 20});
    }); 
    test('success causes enhancement increase by 1', () => {
        expect(enhancer.success({durability: 50, name: '[+1]itemName', enhancement: 1, baseName: 'itemName', namePrefix: '[+1]'})).toEqual({durability: 50, name: '[+2]itemName', enhancement: 2, baseName: 'itemName', namePrefix: '[+2]'});
    }); 
    test('name updated to reflect new success level', () => {
        expect(enhancer.success({durability: 50, namePrefix: '[DUO]', baseName:'itemName', name: '[DUO]itemName', enhancement: 17})).toEqual({durability: 50, namePrefix: '[TRI]', baseName:'itemName', name: '[TRI]itemName', enhancement: 18});
    }); 
    test('enhancement display changes from numerals to PRI at enhancement level 16', () => {
        expect(enhancer.success({durability: 50, namePrefix: '[+15]', baseName:'itemName', name: '[+15]itemName', enhancement: 15})).toEqual({durability: 50, namePrefix: '[PRI]', baseName:'itemName', name: '[PRI]itemName', enhancement: 16});
    }); 
